...
def update_language(language_code):
    config_file_path = "Settings/Config/Translates.py"
    ...
    with open(config_file_path, "w", encoding="utf-8") as file:
        for line in lines:
            if line.startswith("LANGUAGE ="):
                file.write(f"LANGUAGE = '{language_code}'\n")
            else:
                file.write(line)
...
def parameters_menu():
    while True:
        ...
        print(f"{secondary}[{primary}2{secondary}] {primary}Langues")
...
def languages_menu():
    while True:
        print(f"\n{primary}--- Langues ---\n")
        print(f"{secondary}[{primary}1{secondary}] {primary}Français")
        print(f"{secondary}[{primary}2{secondary}] {primary}English")
        print(f"{secondary}[{primary}3{secondary}] {primary}Espagnol")

        choice = input(f"\n{tr('Choice')} -> {reset}")

        if choice == '1':
            update_language('fr')
            print(f"\n{primary}{tr('LanguageChangeFrench')}\n")
            Continue()
            Reset()
        elif choice == '2':
            update_language('en')
            print(f"\n{primary}Language has been changed to English.\n")
            Continue()
            Reset()
        elif choice == '3':
            update_language('es')
            print(f"\n{primary}Language has been changed to Espagnol.\n")
            Continue()
            Reset()
        else:
            ErrorChoice()
